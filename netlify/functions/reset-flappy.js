// Admin-only: clear Flappy Banana scores so a competition can be ended / restarted.
...
// Pre-launch this clears test runs; post-launch the non-destructive way to start
// fresh is to set new competition dates in admin.
...
exports.handler = async function (event) {
  ...
  if (scope === 'current') {
    ...
    // period_start is NOT NULL, so this matches every row (including null player_tag rows).
    filter = `period_start=not.is.null`;
  }

  try {
    const r = await fetch(`${sbUrl}/rest/v1/flappy_scores?${filter}`, {
      method: 'DELETE',
    ...
    return { statusCode: 500, body: JSON.stringify({ error: 'reset-flappy failed', detail: String(e).slice(0, 160) }) };
  }
};
