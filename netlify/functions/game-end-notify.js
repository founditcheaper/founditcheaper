...
exports.handler = async function () {
  ...
  let players = [];
  try {
    const r = await fetch(`${sbUrl}/rest/v1/game_scores?week_start=eq.${encodeURIComponent(start)}&select=username,player_tag,email,week_score&order=week_score.desc&limit=25`, { headers: H });
    const rows = await r.json();
  ...
  if (winner) {
    html += `<p style="font-size:15px;margin:0 0 12px">🏆 <strong>Winner: ${esc(winner.username)}</strong> <span style="color:#888">#${esc(winner.player_tag || '?')}</span> — <strong>${winner.week_score} pts</strong><br><span style="color:#555">${esc(winner.email || '')}</span></p>`;
  ...
  try {
    ...
    console.error('[game-end-notify] email send failed:', e.message);
